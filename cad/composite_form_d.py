#!/usr/bin/env python3
"""composite_form_d.py — renders_form_d (透明底) → renders_form_d_white (白底 RGBA + 软接触投影)
思路复用 composite_form_a.py（不改原文件）：白底合成 + alpha 并集 + 五视图 contact sheet。
"""
import os
import numpy as np
...
def main():
    src = os.path.join(BASE, "renders_form_d")
    dst = os.path.join(BASE, "renders_form_d_white")
    ...
    d.text((20, 12), "LiteCool S1 - Form D Serious Tower (OD90 x H230 squircle, OD110 weighted base, top-slope display)", fill=(60, 60, 65, 255))
