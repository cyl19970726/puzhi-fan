#!/usr/bin/env python3
"""composite_form_a.py — renders_form_a (透明底) → renders_form_a_white (白底 RGBA + 软接触投影)
思路复用 composite_renders.py（不改原文件）：白底合成 + alpha 并集 + 五视图 contact sheet。
"""
import os
import numpy as np
...
def main():
    src = os.path.join(BASE, "renders_form_a")
    dst = os.path.join(BASE, "renders_form_a_white")
    ...
    d.text((20, 12), "LiteCool S1 - Form A Desktop AC (W220 x H140 x D110 + weighted skid base)", fill=(60, 60, 65, 255))
